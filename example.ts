...
import { BeerXML, volume, weight, temperature, gravity } from './src/index.js';
...
console.log(`Batch Size: ${myRecipe.batchSize}L (${volume.litersToGallons(myRecipe.batchSize).toFixed(2)} gal)`);
...
console.log(`Batch size: ${myRecipe.batchSize}L = ${batchSizeGallons.toFixed(2)} gallons`);

const grainWeightLbs = weight.kgToLbs(myRecipe.fermentables[0]?.amount ?? 0);
console.log(`Grain weight: ${myRecipe.fermentables[0]?.amount}kg = ${grainWeightLbs.toFixed(2)} lbs`);
